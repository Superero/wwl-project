...
<title>Ward Wide Learning - article</title>
...
<link rel="stylesheet" href="{{ asset('style/insights.css') }}">
...
<section class="insights-hero" id="insights-top" style="padding-bottom:0;">
  <div class="wrap" style="max-width:820px;">
    <span class="kicker reveal">
      {{-- <i data-lucide="{{ $article->icon }}" aria-hidden="true"></i> --}}
       {{ $article->category }}
    </span>
    <h1 class="reveal reveal-delay-1" style="max-width:100%;">{{ $article->title }}</h1>
    <p class="lead reveal reveal-delay-2">{{ $article->excerpt }}</p>
    <span class="featured-meta reveal reveal-delay-3">
      {{ $article->reading_time }} min de lecture · {{ $article->published_at?->translatedFormat('d F Y') }}
    </span>
  </div>
</section>
  ...
  <div class="wrap" style="max-width:820px;">
    @if($article->image)
      <img src="{{ Storage::url($article->image) }}" alt="{{ $article->title }}"
           style="width:100%;border-radius:var(--radius-lg);border:1px solid var(--border);margin-bottom:40px;">
    @endif
    ...
    <div style="margin-top:48px;">
      <a href="{{ route('insights') }}" class="btn-ghost">
        <i data-lucide="arrow-left" aria-hidden="true"></i> Retour aux insights
      </a>
    </div>
  </div>
</section>
...
<script src="{{asset('scripts/insights.js')}}"></script>
...
<style>
  .article-content{font-size:16.5px;color:var(--text-primary);line-height:1.8;}
  .article-content h2{font-size:24px;margin:36px 0 14px;}
  .article-content h3{font-size:19px;margin:28px 0 12px;}
  .article-content p{margin-bottom:18px;color:var(--text-secondary);}
  .article-content ul, .article-content ol{margin:0 0 18px 22px;color:var(--text-secondary);}
  .article-content a{color:var(--primary);text-decoration:underline;}
  .article-content img{width:100%;border-radius:var(--radius-md);margin:24px 0;}
  .article-content blockquote{
    border-left:3px solid var(--secondary);padding:4px 20px;margin:24px 0;
